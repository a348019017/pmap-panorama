...
class Viewer {
  ...
  constructor(viewer) {
    ...
    this._delegate = viewer; // Initialize the viewer
    ...
    this._dcContainer = DomUtil.create("div", "dc-container", viewer.container); //Register the custom container
  ...
  get resolution() {
    ...
    let min = Transform.transformWindowToWGS84(new Cesium.Cartesian2((width / 2) | 0, height - 1), this);
    let max = Transform.transformWindowToWGS84(new Cesium.Cartesian2((1 + width / 2) | 0, height - 1), this);
    if (!min || !max) {
      return 1;
    }
  ...
  _addLayerGroup(layerGroup) {
    if (layerGroup?.layerGroupEvent && !Object(this._layerGroupCache).hasOwnProperty(layerGroup.id)) {
